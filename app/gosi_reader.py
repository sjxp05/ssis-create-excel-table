...
def load_anchors(path=ANCHORS_PATH):
    if not os.path.exists(path):
        ...
        with open(path, "w", encoding="utf-8") as f:
            json.dump(
                {"_안내": _ANCHORS_GUIDE, **DEFAULT_ANCHORS},
                f,
                ensure_ascii=False,
                indent=2,
            )
    ...
    for name in DEFAULT_ANCHORS:
        ...
        if not (isinstance(a.get("제목"), str) and a["제목"].strip()):
            raise ValueError(
                f"'{name}' 앵커의 '제목'이 비어있거나 잘못되었습니다: {path}"
            )
...
def _validate(prices):
    ...
    g = prices["방문간호"]
    if not (
        g["30분미만"]["금액"] < g["30분이상60분미만"]["금액"] < g["60분이상"]["금액"]
    ):
        raise ValueError(
...
if __name__ == "__main__":
    import sys
    from app.config import YEAR
    from app.param_store import DEFAULT_PATH as OUT_DEFAULT_PATH, save_params

    SRC_DEFAULT_PATH = f"hwpx_files/{YEAR}_고시.hwpx"

    # 윈도우 콘솔 인코딩과 무관하게 한글이 깨지지 않도록
    sys.stdout.reconfigure(encoding="utf-8")

    # argv[1]로 고시의 위치를 지정하지 않아도 연도에 맞게 자동으로 읽어올 수 있게 함
    src = os.path.basename(sys.argv[1]) if len(sys.argv) >= 2 else SRC_DEFAULT_PATH
    try:
        prices = read_gosi_prices(src)
    except FileNotFoundError:
        print(f"해당 경로에 고시 파일이 존재하지 않습니다: {src}")
        sys.exit(1)

    # 추출 결과는 항상 파라미터 파일로 이동
    out = sys.argv[2] if len(sys.argv) >= 3 else OUT_DEFAULT_PATH
    path = save_params(prices, src, out)
