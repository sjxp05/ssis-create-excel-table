# -*- coding: utf-8 -*-
"""결제단가표의 서비스 마스터와 단가 계산 규칙

고시에는 '활동보조 시간당 17,270원' 같은 금액만 있고,
결제 시스템이 쓰는 코드(ST0001, SK0001 등)와 시간대 전개 규칙은 없다.
이 모듈이 그 사이를 잇는다.

    params/단가_파라미터.json  (고시에서 추출한 금액, 출처 포함)
            x
    SERVICE_MASTER  (서비스유형/종류/시간대 코드 체계 = 시스템 정의)
            |
            v
    서비스 단가표: (유형 x 종류 x 시간대)별 단위기준단가/심야단가 + 계산 근거

코드 체계(ST/SK)는 고시가 아니라 결제 시스템의 정의이므로 상수로 두되,
실제 결제단가 샘플과 대조하는 검증 함수(verify_against_sample)를 함께 둔다.

단독 실행(서비스 단가표 확인 + 샘플 검증):
    python -m app.payment_master [검증용 결제단가 샘플.xlsx]
"""

from app.gosi_reader import show_price
from app.param_store import load_params

# ── 사업 고정값: 결제단가표 전체 행에서 동일 (샘플 파일에서 확인) ──
BUSINESS = {
    "사업구분": "HW01",
    "사업구분명": "장애인활동지원",
    "사업유형ID": "HWG001",
...
# ── 서비스 마스터: 유형/종류 코드와 단가 계산 방식 ──
# "단가" 계산 방식 두 가지:
#   시간비율   : 기준 금액 하나에 시간대별 비율을 곱해 파생 (활동보조/방문목욕/지시서)
#   시간대별금액: 시간대마다 고시에 금액이 따로 명시됨 (방문간호)
# "원천"은 파라미터 모음집에서의 위치 (서비스, 항목) 키.
SERVICE_MASTER = [
    {
        "서비스유형ID": "ST0001",
        "서비스유형명": "활동보조",
...
def build_service_prices(prices):
    """파라미터 모음집(고시 금액) x 서비스 마스터 -> (유형 x 종류 x 시간대) 단가표
...
def _validate_rows(prices, rows):
    """조합 수, 금액 상식, 심야=공휴일 전제를 확인. 어긋나면 에러."""
    ...
    # -> 고시에서 심야와 공휴일 금액이 같다는 전제 위에 서 있으므로, 달라지면 멈춘다
    h = prices["활동보조"]
    if h["심야"]["금액"] != h["공휴일"]["금액"]:
        raise ValueError(
...
def verify_against_sample(rows, sample_xlsx):
    """생성한 서비스 단가표를 실제 결제단가 샘플과 전수 대조한다."""
    import pandas as pd
